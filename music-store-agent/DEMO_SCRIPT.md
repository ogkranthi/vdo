...
| 8 | Subagent tool calls don't appear in the supervisor's message list (isolated context), so a trajectory evaluator reading `result["messages"]` silently under-counts — `expected_tools_used` flatlined at 0.33 across two experiments before the evaluator itself was diagnosed | 30 min | Capture the trajectory with a `BaseCallbackHandler` on the run config — callbacks fire for nested runs. Meta-lesson for the demo: evaluate your evaluators; a metric that never moves is a bug |
| 9 | LLM-as-judge correctness swung 0.92 → 0.67 between experiments running *identical agent code* — judge strictness varies run to run on borderline answers (e.g. it penalized a capability answer for "comparing to the reference" oddly) | — | Deterministic trajectory scores are stable (0.92); treat judge scores as a trend + read the attached reasoning, and calibrate the judge prompt/references over time. Honest Q&A ammo for "how reliable is LLM-as-judge?" |
